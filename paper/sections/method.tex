...
hierarchy (\texttt{generalizes}).  These seven types form a basis analogous to
morphism classes in category theory: every scientific relation we encountered
maps naturally to one of these types.  The fixed vocabulary enables cross-domain
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Decomposability}: each component
    ...
    ablation.
  \item \textbf{Directional monotonicity}: each component responds
    predictably to edge addition---compressibility generally decreases (more
    cross-edges reduce spanning fraction), coherence increases when the new
    edge closes a type-consistent triangle, symmetry increases when the edge
    balances entity-type distributions, and generativity increases when the
    edge adds learnable relational signal.  The Harmony score thus captures
    the \emph{net} structural effect of a mutation across these competing
    pressures.
\end{enumerate}
...
symmetry operationalises Noether's theorem intuition that good theories
exhibit invariance across structurally equivalent entities; and generativity
