...
The seven relation types cover dependency, derivation, equivalence,
correspondence, explanation, contradiction, and hierarchy---a minimal set
inspired by morphism classes in category theory
(Appendix~\ref{app:edge-type-rationale}).
...
only when $\varepsilon > 0$ for hybrid analysis (Appendix~\ref{app:freq}).
We use a linear composition for interpretability: each axis's contribution
to a Harmony score is identifiable.  Multiplicative compositions
(geometric means) penalise low values disproportionately and would
conflate sub-axis weakness with overall metric failure.  The equal-weight
default reflects the absence of theoretical priors over the four
desiderata; a calibration weight grid (Appendix~\ref{app:calibration-gate})
explores this neighbourhood.

\paragraph{Compressibility.}
An MDL proxy combining edge-type entropy and spanning structure:
\begin{equation}\label{eq:compress}
  ...
  \right),
\end{equation}
where $H(\mathbf{p})$ is the Shannon entropy of the edge-type distribution
and the spanning fraction counts BFS spanning-tree edges over an undirected
view of~$G$.\footnote{The denominator $\log_2 7$ matches the cardinality of
the EdgeType enum (Section~\ref{sec:typed-kg}); for extensibility, an
implementation should use $\log_2|\text{EdgeType}|$.  This bounds
$H(\mathbf{p})/\log_2 7 \in [0,1]$ for any KG drawn from the current
vocabulary.}

\paragraph{Coherence.}
Path-semantic consistency via triangle type-agreement and contradiction rate:
\begin{equation}\label{eq:cohere}
  ...
  \right).
\end{equation}
Triangle agreement is satisfied when the closing edge $r_{ac}$ has
\emph{exactly} one of the two path types $\{r_{ab}, r_{bc}\}$;
neither inclusion in the union of all edge types nor consistency
with a separate semantic compatibility table is required.

\paragraph{Symmetry.}
Intra-type behavioural consistency via Jensen--Shannon divergence.  For each
entity type $\tau$, let $\mathbf{q}_v \in \Delta^6$ be entity $v$'s outgoing
edge-type distribution:
\begin{equation}\label{eq:symm}
  \Symm(G) = \frac{\sum_\tau n_\tau \cdot \bigl(1 - \frac{1}{n_\tau}
    \sum_{v \in \tau} \text{JS}(\mathbf{q}_v, \bar{\mathbf{q}}_\tau)\bigr)}
    {\sum_\tau n_\tau}.
\end{equation}

\paragraph{Generativity.}
Link-prediction learnability via DistMult \citep{yang2015distmult} on 20\%
masked edges:
...
(full DistMult hyperparameters, including margin, learning rate,
negative sampling rate, and mask ratio, in
...
$\Cost(\Delta) = 1/\max(|E|,1)$ for edge mutations,
$1/\max(|V|,1)$ for entity mutations.
The penalty weight $\lambda = 0.1$ discourages trivially large proposals.
The Harmony score is bounded in $[0,1]$, decomposes into independently
computable components, and exhibits empirical directional monotonicity
(Appendix~\ref{app:formal-properties}).
...
Algorithm~\ref{alg:harmony} summarises a single generation
(Figure~\ref{fig:architecture}).  The loop runs for $T_{\max} = 20$ generations
per experiment, checkpointing state after each generation to enable resumption.
...
\begin{figure}[t]
  ...
  \caption{Overview of the Harmony island-model search framework.  Four islands
    ...
    migration (every $M=10$ generations) transfers each island's best proposal
    to the next island in a ring topology ($i \to (i{+}1) \bmod 4$).}
