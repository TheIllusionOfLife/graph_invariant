...
social network analysis. However, the discovery of \emph{new} invariants that
capture previously unrecognized structural relationships remains a largely
...
evolution~\citep{whitley1999island} and MAP-Elites quality-diversity
search~\citep{mouret2015mapelites} to discover interpretable graph invariants.
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  \item An open-source framework for LLM-driven graph invariant discovery with
  ...
  \item Systematic evaluation across four experiment configurations with
    ...
    demonstrating that LLM-discovered formulas are competitive with statistical
    and symbolic regression baselines while remaining interpretable.
\end{itemize}
