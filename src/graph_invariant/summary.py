...
from .evaluation import (
    best_candidate,
    dataset_fingerprint,
    evaluate_bound_split_with_predictions,
...
def write_phase1_summary(
    ...
    # Precompute raw predictions to avoid re-evaluation
    y_pred_val = evaluator.evaluate(best.code, features_val)
    y_pred_test = evaluator.evaluate(best.code, features_test)
    y_pred_train = evaluator.evaluate(best.code, features_train)
    y_pred_sanity = evaluator.evaluate(best.code, features_sanity)

    val_metrics = evaluate_split_with_predictions(y_pred_val, y_true_val, known_val)
    test_metrics = evaluate_split_with_predictions(y_pred_test, y_true_test, known_test)
    train_metrics = evaluate_split_with_predictions(y_pred_train, y_true_train)
    sanity_metrics = evaluate_split_with_predictions(y_pred_sanity, y_sanity)

    novelty_ci = {
        "validation": _novelty_ci_for_split(y_pred_val, known_val, 17, cfg),
        "test": _novelty_ci_for_split(y_pred_test, known_test, 29, cfg),
    }

    is_bounds = cfg.fitness_mode in ("upper_bound", "lower_bound")

    if is_bounds:
        val_bound_metrics = evaluate_bound_split_with_predictions(
            y_pred_val,
            y_true_val,
            fitness_mode=cfg.fitness_mode,
            tolerance=cfg.bound_tolerance,
        )
        test_bound_metrics = evaluate_bound_split_with_predictions(
            y_pred_test,
            y_true_test,
            fitness_mode=cfg.fitness_mode,
            tolerance=cfg.bound_tolerance,
