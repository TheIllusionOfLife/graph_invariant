...
\emph{compositions} of known invariants/features (functions that are themselves invariant to vertex relabeling) that capture structural
...
Unlike pure symbolic regression, LLMs act as a structured mathematical prior: they compose known constructs (harmonic means, Moore bounds, min-of-bounds) rather than performing blind operator search, enabling candidates that are plausible starting points for proof.
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  ...
  \item Systematic evaluation on ASPL as a benchmark target, demonstrating
    strong (though not best-in-class) correlation with statistical and symbolic
    regression baselines across five random seeds with out-of-distribution
    validation.
  \item A \emph{bounds mode} for empirical conjecture generation: the system
    discovers candidate mathematical inequalities (e.g., compositions of
    path-graph and Moore bounds for ASPL) that are plausible starting points for
    formal proof.
\end{itemize}
