...
Training ($m = 50$ graphs) and validation/test ($m = 200$ graphs each) sets
are sampled from five generative families---ER, BA, WS, RGG, SBM---with node
counts $|V| \in [30, 100]$ and deterministic seeding (seed $= 42$ unless
otherwise noted).
...
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  \item \textbf{Large random} ($m = 100$ graphs): Same five families but with
    $|V| \in [200, 500]$.
  \item \textbf{Extreme parameters} ($m = 50$ graphs): Extreme densities and
    degree distributions with $|V| \in [50, 200]$.
