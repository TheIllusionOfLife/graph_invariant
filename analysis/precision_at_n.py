...
def recall_at_n(
    ...
    """Compute Recall@N: fraction of withheld edges found in top-N proposals.
    ...
    Returns:
        Fraction of withheld edges matched by the top-N proposals.
    """
    if not withheld or not proposals_ranked or n <= 0:
        return 0.0
    ...
    if not unique_top:
        return 0.0
    found = sum(1 for triple in unique_top if triple in withheld_set)
    return float(found) / len(withheld_set)
