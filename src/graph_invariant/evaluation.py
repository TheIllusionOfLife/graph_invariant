...
def evaluate_split(
    ...
    return evaluate_split_with_predictions(y_pred_raw, y_true, known_invariants)


def evaluate_bound_split_with_predictions(
    y_pred_raw: list[float | None],
    y_true: list[float],
    fitness_mode: str,
...
def evaluate_bound_split(
    ...
    y_pred_raw = evaluator.evaluate(code, features_list)
    return evaluate_bound_split_with_predictions(
        y_pred_raw, y_true, fitness_mode, tolerance=tolerance
    )
