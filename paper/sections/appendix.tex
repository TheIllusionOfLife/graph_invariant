...
\midrule
Linear algebra     & 17  & 45  & 5 & --- & Hand-built \\
Periodic table     & 22  & 58  & 4 & --- & Hand-built \\
Astronomy          & 20  & 52  & 6 & --- & Hand-built \\
Physics            & 18  & 48  & 5 & --- & Hand-built \\
Materials science  & 19  & 50  & 5 & --- & Hand-built \\
Wikidata Physics   & ---  & --- & --- & --- & Wikidata \\
Wikidata Materials & ---  & --- & --- & --- & Wikidata \\
\bottomrule
\end{tabular}
\end{table*}

\noindent Edge type counts and Wikidata domain statistics will be populated from
\texttt{analysis/reproducibility\_table.py} after live experiment runs.  The
dataset splitting protocol (Section~\ref{sec:experiments}) applies identically
to all seven domains: 10\% hidden, then 80/10/10 train/val/test on the
remaining 90\%.
...
GPU).  Each domain completes 20~generations in approximately 10~minutes of
wall-clock time (including LLM inference via locally served Ollama).  The total
compute for the seven reported domains (five hand-curated + two Wikidata) is
under 2~CPU-hours for the main experiments.  Factor decomposition baselines
(LLM-only, No-QD) require additional LLM calls ($\sim$4,600 total).
