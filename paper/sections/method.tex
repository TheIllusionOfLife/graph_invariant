...
\begin{figure}[t]
  ...
  \fbox{\parbox{0.9\textwidth}{\centering\small
    ...
    \\[2pt]
    \textit{[Placeholder: will be replaced with vector diagram from results]}
  }}
...
models (SBM)---with node counts $n \in [30, 100]$. Each graph is augmented with
a feature dictionary containing pre-computed structural properties: node count
$n$, edge count $m$, density, degree statistics (mean, max, min, std), average
clustering coefficient, transitivity, degree assortativity, triangle count, and
the sorted degree sequence. The dataset is split into train ($n_{\text{train}}
= 50$), validation ($n_{\text{val}} = 200$), and test ($n_{\text{test}} = 200$)
sets using deterministic seeding for reproducibility.

Target values are computed per graph for the specified invariant (e.g.,
\texttt{average\_shortest\_path\_length} or \texttt{algebraic\_connectivity}).
...
nodes in the candidate function body and $c_{\max} = 50$. This penalizes
complex expressions while remaining differentiable in candidate selection.
