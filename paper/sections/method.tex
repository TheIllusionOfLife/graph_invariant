...
We present the Harmony framework in three parts: the typed KG schema and Harmony
metric (Section~\ref{sec:metric}), the proposal schema and validation
(Section~\ref{sec:proposals}), and the island-model search loop
...
Jensen--Shannon distance (base~2), bounded in $[0,1]$.
...
(merge the top two proposals), \emph{refinement}, and \emph{novel} (invent from
...
$G_{\max} = 20$ generations per experiment, checkpointing state after each
...
\FOR{each island $I_k$}
  \STATE $\sigma_k \gets$ \textsc{Strategy}$(k)$ \COMMENT{refinement / combination / novel}
