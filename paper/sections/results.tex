...
score of 0.514, demonstrating that the system can discover non-trivial
...
\begin{table}[t]
  ...
  \caption{Summary of results across four experiment configurations. Spearman
    ...
    Benchmark reports mean $\pm$ std across 5 seeds.}
...
$\rho = 0.947$ on the test set, competitive with PySR ($\rho = 0.975$) and
random forest ($\rho = 0.951$), while providing interpretable closed-form
expressions. Linear regression achieves $\rho = 0.975$, indicating that ASPL
is well-approximated by linear combinations of graph statistics.

\begin{table}[t]
  \centering
  \caption{Comparison of LLM-discovered formulas with baselines on
    \texttt{average\_shortest\_path\_length}. Val and Test Spearman $\rho$ reported.}
  \label{tab:baselines}
  \begin{tabular}{lcc}
    \toprule
...
ASPL experiment converges from a best score of 0.426 to 0.553 over 30
generations, while the upper-bound experiment shows the steepest improvement
(0.228 $\to$ 0.453).
...
differ qualitatively from the stochastic training distribution.
...
stochastic nature of LLM generation. One seed (seed 55) meets the success
threshold with test $\rho = 0.953$.
...
\begin{figure}[t]
  ...
  \caption{Distribution of Spearman $\rho$ across 5 benchmark seeds.
    Validation: $0.927 \pm 0.011$; Test: $0.921 \pm 0.027$. The tight
...
$(n+1)/3$ with Moore-bound arguments, producing a provably valid inequality
for most graph families.
...
\begin{table}[t]
  ...
  \label{tab:formulas}
  \small
  \begin{tabular}{p{2.2cm}p{6.5cm}c}
    \toprule
    Experiment & Key Formula Components & Val $\rho$ \\
    \midrule
    MAP-Elites ASPL & $\frac{\sqrt{n}}{d+1} \cdot \frac{1}{\delta} \cdot (1+C)^{0.6} \cdot (1+T)^{0.5} \cdot \frac{d_H}{d} \cdot \ldots$ & 0.935 \\[4pt]
