...
Table~\ref{tab:metrics} compares Hits@10 and MRR across the three discovery
domains after applying top proposals from the MAP-Elites archive to the base KG.
...
per domain in \textbf{bold}.}\label{tab:metrics}
...
9--15\% across all three domains.  The improvement is consistent across all
ranking cutoffs (Hits@3, Hits@1) and MRR, indicating that the proposals inject
structurally meaningful edges rather than noise.
...
domains.  The MAP-Elites archive achieves 40--60\% coverage of the $5 \times 5$
grid (10--15 of 25 cells occupied), indicating that the island-model search
produces diverse proposals spanning multiple simplicity--gain trade-offs.
