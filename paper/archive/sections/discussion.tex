...
0.030$), diversity archiving yields a consistent improvement. The archive
...
The LLM-driven self-correction (SC) loop recovers mathematical intuition from
...
restructuring the mathematical expression. A controlled ablation
(Table~\ref{tab:sc_ablation}, Appendix) with 3 matched seeds confirms this
quantitatively: SC raises mean validation $\rho$ by $+0.045$ (7.5$\times$
the SC-on validation std) while reducing cross-seed standard deviation by
3$\times$, indicating that the repair loop stabilises convergence without
degrading held-out generalisation.
